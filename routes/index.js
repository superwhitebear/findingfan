...
router.post('/search',function(req, res, next){
	if (req.session.username){
		...
		var db = req.db;
   		...
   		if(name.length>0 && interested.length>0){
   			collection.find({'username': name,'interrested': interested},{}, function(e, docs) {
   				res.render('list', {docs: docs});
   			});
   		} else if(name.length>0 && interested.length==0){
   				collection.find({'username': name},{}, function(e, docs) {
   					res.render('list', {docs: docs});
   				});
   		} else if(name.length==0 && interested.length>0){
   				collection.find({'interrested': interested},{}, function(e, docs) {
   					res.render('list', {docs: docs});
   				});
   		}
	}else{
